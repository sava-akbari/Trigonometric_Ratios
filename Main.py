...
# Tab 4 - Program 3
# برگه چهارم - برنامه‌ی سوم؛ مقایسه‌ی مقدار نسبت‌های مثلثاتیا
tab_program3 = ttk.Frame(tab_control)
tab_control.add(tab_program3, text='مقایسه‌ی مقدار دو نسبت مثلثاتی')

# Create title label
# ساختن یک عنوان برای برنامه‌ی دوم
program1_label = tk.Label(tab_program3, text="مقایسه‌ی مقدار دو نسبت مثلثاتی", font=("B Titr", 24), fg="#7300bf")
program1_label.pack(padx=10, pady=10)

#Create a canvas object
# ایجاد فضا برای اضاقه کردن توضیحات برنامه‌ی سوم
Canvas= tk.Canvas(tab_program3, width= 570, height= 200, bg="#c0ecbf")

#Add a text in Canvas
# اضافه کردن خود متن
Canvas.create_text(280,95, text="""       این برنامه از کاربر یک زاویه بر حسب درجه و یک تابع مثلثاتی دریافت می‌کند 
         .سپس مقدار مثلثاتی مربوطه را به کمک کتابخانه‌های مربوطه محاسبه می‌کند   
           نتیجه را مقایسه کرده و به کاربر نمایش می‌دهد. این برنامه همچنین هرگونه
                                               .ورودی نامعتبر را با چاپ یک پیام خطا کنترل می‌کند""", fill="navy", font=("B Titr", 14))
Canvas.pack()
...
def run_Comparison():
    subprocess.call(["python","Trigonometric_Ratios\Comparison.py"])

# Add button to run other project
# ایجاد دکمه برای اجرای برنامه
run_project_button = tk.Button(tab_program3, text="اجرای‌ برنامه", font=("B Titr", 14), fg="#7300bf", command=run_Comparison)
run_project_button.pack(padx=10, pady=10)
